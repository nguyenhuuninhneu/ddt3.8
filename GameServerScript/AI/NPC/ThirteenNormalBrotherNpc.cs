...
namespace GameServerScript.AI.NPC
{
    public class ThirteenNormalBrotherNpc : ABrain
    {
        ...
        private void method_0()
        {
			...
			else
			{
				...
				foreach (Player allLivingPlayer2 in base.Game.GetAllLivingPlayers())
				{
					...
					allLivingPlayer2.AddEffect(new AddDamageTurnEffect(int_1, 2), 7000);
					allLivingPlayer2.AddEffect(new AddGuardTurnEffect(int_2, 2), 7000);
				}
				(base.Game as PVEGame).SendObjectFocus(nkDsoVfybVd, 0, 9000, 0);
				nkDsoVfybVd.Say("Có chuyện gì vậy? Cúng tế bị sai cmnr.", 0, 10500, 2000);
				base.Body.PlayMovie("walk", 8000, 0);
				base.Body.BoltMove(1604, 594, 8500);
				base.Body.CallFuction(method_1, 13000);
			}
			nkDsoVfybVd.Delay = base.Game.GetLowDelayTurn() - 1;
        ...
        public override void OnBeginSelfTurn()
        {
			...
			if (nkDsoVfybVd == null)
			{
				SimpleBoss[] array = ((PVEGame)base.Game).FindLivingTurnBossWithID(ootsomGdKu0);
				if (array.Length != 0)
				{
					nkDsoVfybVd = array[0];
				}
