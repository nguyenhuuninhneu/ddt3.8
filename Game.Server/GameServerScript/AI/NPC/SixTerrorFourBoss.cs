...
namespace GameServerScript.AI.NPC
{
    public class SixTerrorFourBoss : ABrain
    {
        ...
        private void ShootAttack()
        {
            ...
            for (int i = 0; i < players.Count; i++)
            {
                if (players[i] != null)
                {
                    ...
                    if (Body.ShootPoint(mtX, players[i].Y, 61, 1000, 10000, 3, 1, delay + 2200))
                    {
        ...
        private void PlayerAddArmor(List<Player> player)
        {
            foreach (Player p in player)
            {
                p.AddEffect(new AddGuardEquipEffect(2, 250, true), 2000);
            }
            //m_bluePlayers = new List<Player>();
        }
