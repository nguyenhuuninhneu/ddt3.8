...
namespace GameServerScript.AI.NPC
{
    public class ThirteenNormalDevilBoss : ABrain
    {
        ...
        private void CreateGlobalAttack()
        {
            ...
            Body.MoveTo(900, 400, "fly", 1000, "", 12, RunGlobalAttack);
        }
        ...
        {
            phyObjects.Add(((PVEGame)Game).CreateLayerTop(500, 300, "", "asset.game.4.heip", "", 1, 0));
        }

        private void CreateDamageFire()
        {
            Body.Say(Game.ListPlayersName() + ", ta muốn ăn bít tết.", 1, 1000);

            Body.PlayMovie("beatD", 2000, 4000);

            Body.CallFuction(CreateFireEffect, 4000);
            Body.CallFuction(MakeFriendBossSay, 6000);
        }

        private void MakeFriendBossSay()
        {
            ((PVEGame)Game).SendFreeFocus(m_friendNpc.X, m_friendNpc.Y - 150, 0, 0, 0);
            m_friendNpc.Say("Quỷ khăn hồng đáng ghét! Chớ để chúng đến gần ta, ta không muốn biến thành bít tết đâu!", 1, 1500);
        }
        ...
        private void CreateFireEffect()
        {
            foreach (Player p in Game.GetAllLivingPlayers())
            {
                ...
                ((PVEGame)Game).SendFreeFocus(p.X, p.Y - 100, 0, 0, 0);
            }
        }
        ...
        {
            m_targer = Game.FindRandomPlayer();
            Body.Say("Ta đến đây!", 1, 1000);
            Body.MoveTo(m_targer.X, m_targer.Y - 100, "fly", 1500, "", 12, AttackTarget);
        }
        ...
        {
            int x = Game.Random.Next(400, 1700);
            int y = Game.Random.Next(450, 850);
            Body.MoveTo(x, y, "fly", 1000, "", 12, CallAttackFriendlyBoss);
        }
        ...
        private void CallAttackFriendlyBoss()
        {
            ...
            else if (phyIronFire.Count > 0)
            {
                ...
                if (rangePlayers.Count >= 4)
                {
                    ...
                    Body.CallFuction(ClearAllFireDamageEffect, 1000);
                    //Body.CallFuction(ClearAllFireDamageEffect, 2500);
                    Body.CallFuction(CreateFireIronFriendBoss, 2000);
                    Body.CallFuction(DamageFireIronFriendBoss, 4000);


                    m_friendNpc.PlayMovie("ready", 5500, 0);
                    m_friendNpc.PlayMovie("standC", 7200, 2000);

                    Body.CallFuction(UpdateStatusDamage, 9200);
                    Body.CallFuction(ClearAllFireDamageEffect, 6500);
                }
                else
                {
        ...
        private void DamageFireIronFriendBoss()
        {
            foreach (PhysicalObj physical in phyIronFire)
            {
                physical.PlayMovie("beatA", 0, 0);
            }
            m_friendNpc.AddBlood(-m_friendNpc.MaxBlood / 100 * 12, 1);
        }

        private void CreateFireIronFriendBoss()
        {
            phyIronFire.Add(((PVEGame)Game).Createlayer(m_friendNpc.X - 100, m_friendNpc.Y - 170, "", "asset.game.ten.laotie2", "", 1, 0));
            phyIronFire.Add(((PVEGame)Game).Createlayer(m_friendNpc.X - 80, m_friendNpc.Y - 200, "", "asset.game.ten.laotie2", "", 1, 0));

            phyIronFire.Add(((PVEGame)Game).Createlayer(m_friendNpc.X + 100, m_friendNpc.Y - 170, "", "asset.game.ten.laotie", "", 1, 0));
            phyIronFire.Add(((PVEGame)Game).Createlayer(m_friendNpc.X + 80, m_friendNpc.Y - 200, "", "asset.game.ten.laotie", "", 1, 0));
        }
        ...
        private void CreateBig1000()
        {
            ...
            Body.Say("Ối chà, to và nặng quá!", 1, 1000);
