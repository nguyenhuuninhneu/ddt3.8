...
namespace GameServerScript.AI.NPC
{
	...
	public class TwelveHardFlyThirdBoss : ABrain
	{
		...
		private void ReadyElectricAttack()
		{
			...
			int index = base.Game.Random.Next(0, TwelveHardFlyThirdBoss.ElectricChat.Length);
			base.Body.Say(TwelveHardFlyThirdBoss.ElectricChat[index], 1, 0);
		...
		private void HoleAttack()
		{
			...
			int index = base.Game.Random.Next(0, TwelveHardFlyThirdBoss.HoleChat.Length);
			base.Body.Say(TwelveHardFlyThirdBoss.HoleChat[index], 1, 0);
			foreach (Player allFightPlayer in base.Game.GetAllFightPlayers())
			{
