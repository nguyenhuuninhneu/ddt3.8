...
public class GamePlayer : IGamePlayer
{
    ...
    public bool AddTemplate(ItemInfo cloneItem, eBageType bagType, int count, eGameView gameView, string Name)
    {
        ...
        if (cloneItem != null)
        {
            ...
            List<ItemInfo> list = new List<ItemInfo>();
            if (!inventory.AddItem(cloneItem))
            {
                list.Add(cloneItem);
            }
            BagFullSendToMail(list);
        ...
        if (WorldMgr.AddPlayer(m_character.ID, this))
        {
            try
            {
                if (LoadFromDatabase())
                {
                    ...
                    //if (Extra.CheckNoviceActiveOpen(NoviceActiveType.UPDATE_FIGHTPOWER))
                    //{
                    ...
                    //if (this.m_character.GP < 39291 || this.m_character.IsFirst <= 18)
                    //{
                    //    this.AddGP(39291 - this.m_character.GP, false);
                    //    this.AddMoney(50000000);
                    //    this.AddGiftToken(50000000);
                    //    this.AddGold(50000000);
                    //    this.m_character.IsFirst = 19;
    ...
    public void OpenAllNoviceActive()
    {
        ...
        using (PlayerBussiness playerBussiness = new PlayerBussiness())
        {
            EventRewardProcessInfo[] userEventProcess = playerBussiness.GetUserEventProcess(PlayerId);
            EventRewardProcessInfo[] array = userEventProcess;
            EventRewardProcessInfo[] array2 = array;
            foreach (EventRewardProcessInfo eventRewardProcessInfo in array2)
            {
                if (eventRewardProcessInfo.ActiveType == 3 || eventRewardProcessInfo.ActiveType == 4)
                {
